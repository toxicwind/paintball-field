<!--
  ...
  Open Source. MIT Licensed. Build it, break it, mod it, sell it.
-->
...
|------|--------------|----------|---------|------------|
| [Spectre](modes/spectre/config.json) | One player per team sees all enemies permanently | 10 min | 4v4–8v8 | Hard |
| [Hunter-Prey](modes/hunter_prey/config.json) | All players get 3s wall vision every 60s | 15 min | 5v5–10v10 | Medium |
| [Ghost](modes/ghost/config.json) | Dead players become invisible spotters for team | 12 min | 6v6 | Hard |
...
|------|--------------|-------------|----------|---------|
| [Capture the Flag](modes/capture_flag/config.json) | Grab enemy flag box, return to base | 2 | 15 min | 5v5–8v8 |
| [Domination](modes/domination/config.json) | Hold zones to reveal enemies inside captured zones | 3–5 | 12 min | 5v5–10v10 |
| [Search & Destroy](modes/search_destroy/config.json) | Plant/defuse bomb at objective sites | 2 | 10 min | 4v4–6v6 |
| [Data Heist](modes/data_heist/config.json) | Hack 3 terminals in sequence | 3 | 15 min | 4v4–6v6 |
...
|------|--------------|----------|---------|
| [Infection](modes/infection/config.json) | BLE proximity infection; survivors see infected | 10 min | 6–16 |
| [VIP Escort](modes/vip_escort/config.json) | Protect VIP across field; enemy sees VIP every 30s | 12 min | 5v5 |
| [Battle Royale](modes/battle_royale/config.json) | Shrinking zone; last standing wins | 15 min | 8–32 |
| [Frontline](modes/frontline/config.json) | Checkpoint respawn; deplete enemy reinforcements | 15 min | 6v6–10v10 |
...
| Mode | Core Mechanic | Duration | Players |
|------|--------------|----------|---------|
| [Overwatch](modes/overwatch/config.json) | Commander sees full map; pings team | 15 min | 4v4 |
| [Tournament](modes/tournament/config.json) | Referee validation, replay, brackets | Varies | 4v4–8v8 |
...
python src/main.py --config configs/default_field.json
```
...
1. **Order PCBs**: Upload `hardware/band_v1.0/gerbers.zip` to JLCPCB. Select 2-layer, 1.6mm, HASL, any color. 5 pieces cost ~$7 shipped.
2. **Order Components**: Use the BOM CSV in `hardware/band_v1.0/BOM.csv` to order from LCSC. Total ~$12 per band in quantity 10.
...
1. **Place AP Nodes**: Mount 6 ESP32-C3-DevKitM-1 boards in IP65 enclosures at the coordinates specified in `server/configs/default_field.json`. Height: 2.5 meters, facing downward.
2. **Configure APs**: Flash `firmware/ap_node/src/main.cpp` to each AP node. Set SSID to `SPECTRE-AP-01` through `SPECTRE-AP-06`.
...
4. **Start Server**: Run `python src/main.py` on the Pi. Verify AP nodes appear in the server logs.
...
```bash
python server/src/calibrate.py --field-size 50x30 --samples 100
```
...
## Case Study: Blitz Paintball, Dacono CO

SpectreBand is currently in pilot planning at **Blitz Paintball** in Dacono, Colorado. This serves as our primary case study and stress-test environment.

### Why Blitz?

Blitz operates **4 distinct fields** on a single property, giving us 4 unique test environments:
...
- **Band BOM**: $12 (v1.0 pilot) / $19.70 (v1.1 production)
- **Payback**: 2 sessions at $8 rental
- **Target margin**: 60% after band cost recovered

See `fields/blitz_dacono/config.json` for the full Blitz field configuration, AP placement coordinates, and per-field calibration targets. Also available at `server/configs/blitz_dacono.json` for server loading.
...
A: Absolutely. Game modes are JSON configuration files in `server/configs/modes/`. Define vision rules, scoring, win conditions, and band behavior. The server handles the rest.
...
<p align="center">
  ...
  <sub>SpectreBand is an open-source project. MIT licensed. Build it, break it, mod it, sell it.</sub>
</p>
